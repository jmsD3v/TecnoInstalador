...
import { Zap, ArrowRight, Star, Search, MapPin, ShieldCheck } from 'lucide-react'
        ...
        {[0, 1].map(i => (
          <button
            ...
            className={cn(
              ...
              active === i
                ? (i === 0 ? 'w-8 bg-orange-400' : 'w-8 bg-emerald-400')
...
function SlideWrapper({ visible, exiting, children }: {
  visible: boolean
  exiting: boolean
  ...
  return (
    <div
      className={cn(
        ...
        !visible && !exiting && 'opacity-0 translate-y-4 z-0 pointer-events-none',
      )}
    >
      {children}
    </div>
  ...
  return (
    <div className="max-w-3xl">
      <div className="inline-flex items-center gap-2 border border-primary/30 bg-primary/10 text-primary text-[11px] sm:text-xs font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-full mb-5 sm:mb-8 tracking-wider uppercase">
        ...
        <span className="hidden sm:inline">Plataforma para profesionales del oficio</span>
      ...
      <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-xl mb-7 sm:mb-10 leading-relaxed">
        ...
        y conectá con nuevos clientes.
      </p>

      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <Button size="xl" asChild className="w-full sm:w-56 justify-center font-bold shadow-lg shadow-primary/25">
          <Link href={isLoggedIn ? '/dashboard' : '/auth/register'}>
            {isLoggedIn ? 'Ir a mi panel' : 'Crear mi perfil gratis'}
            <ArrowRight className="w-5 h-5" />
          </Link>
        </Button>
        <Button size="xl" variant="outline" asChild className="w-full sm:w-56 justify-center">
          <Link href="/buscar">
            Ver instaladores
            <ArrowRight className="w-5 h-5" />
          </Link>
      ...
      <div className="flex items-center gap-4 sm:gap-6 mt-7 sm:mt-10">
        <div className="flex -space-x-2">
          {[
            ...
            <div key={i} className="w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 border-background flex items-center justify-center text-[10px] sm:text-xs font-bold text-white" style={{ background: a.bg }}>
              {a.init}
            </div>
          ))}
        </div>
        <div className="flex items-center gap-1.5 text-xs sm:text-sm text-muted-foreground">
          <div className="flex">
            {[1,2,3,4,5].map(i => <Star key={i} className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-yellow-400 fill-yellow-400" />)}
          </div>
  ...
  return (
    <div className="max-w-3xl">
      <div className="inline-flex items-center gap-2 border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-[11px] sm:text-xs font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-full mb-5 sm:mb-8 tracking-wider uppercase">
        ...
        <span className="hidden sm:inline">Encontrá al profesional que necesitás</span>
      ...
      <p className="text-base sm:text-lg md:text-xl text-muted-foreground max-w-xl mb-7 sm:mb-10 leading-relaxed">
        Electricistas, plomeros, gasistas y más —{' '}
        <span className="text-emerald-400 font-semibold">verificados y con reseñas reales</span>.{' '}
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <Button
          size="xl"
          asChild
          className="w-full sm:w-56 justify-center font-bold shadow-lg shadow-emerald-500/20 bg-emerald-500 hover:bg-emerald-400 text-white"
        >
          <Link href="/buscar">
      ...
      <div className="flex items-center gap-4 sm:gap-6 mt-7 sm:mt-10">
        ...
        <div className="flex items-center gap-1.5 text-xs sm:text-sm text-muted-foreground">
          ...
          <span><span className="text-foreground font-semibold">4.8★</span> rating promedio</span>
        </div>
      </div>
    </div>
  )
